...
class IoTimingTest {
    ...
    fun `multi-region processing completes within time budget`() {
        ...
        for ((i, name) in regions.withIndex()) {
            val data = McaMemoryBuilder.buildMca(
                listOf(
                    McaMemoryBuilder.MemChunk(index = i * 4, inhabited = (i * 1000).toLong(), kind = CompressionKind.RAW),
                    McaMemoryBuilder.MemChunk(index = i * 4 + 1, inhabited = ((i + 1) * 1000).toLong(), kind = CompressionKind.ZLIB),
                    McaMemoryBuilder.MemChunk(index = i * 4 + 2, inhabited = ((i + 2) * 1000).toLong(), kind = CompressionKind.LZ4)
                )
            )
    ...
    fun `parallel processing is not slower than serial baseline`() {
        ...
        // Serial run
        val serialRequest = OptimizerRequest(
            input = world,
            output = out,
            filter = FilterOptions(inhabitedThresholdSeconds = 0),
        ...
        val fs2 = MemoryFS()
        for (root in listOf(world, dim1, dim2)) {
            val newRoot = java.nio.file.Paths.get(root.toString().replace("/mem/timing-parallel", "/mem/timing-parallel2"))
        ...
        val parallelRequest = OptimizerRequest(
            input = world2,
            output = out2,
            filter = FilterOptions(inhabitedThresholdSeconds = 0),
        ...
        if (serialMs > 0 && parallelMs > serialMs * maxSlack) {
            throw AssertionError("Parallel (${parallelMs}ms) is more than ${maxSlack}x slower than serial (${serialMs}ms)")
        }
    }
}
